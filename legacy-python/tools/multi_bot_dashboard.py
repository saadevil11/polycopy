"""
Multi-Bot Unified Dashboard
Monitors multiple Polymarket copy trading bot instances simultaneously
Run with: python tools/multi_bot_dashboard.py
"""
import json
import sqlite3
import sys
...
BOTS_CONFIG = [
    {
        "name": "Bot 1",
        "db_path": "data/trades.db",  # Local database path
        "description": "Primary wallet",
        "color": "#007bff"  # Blue
    },
    {
        "name": "Bot 2", 
        "db_path": "data/trades_bot2.db",  # Second bot database
...
bot_monitors = [BotMonitor(**config) for config in BOTS_CONFIG]
...
def api_bot_detail(bot_id):
    """API endpoint for individual bot details"""
    try:
        if bot_id >= len(bot_monitors):
            return jsonify({'status': 'error', 'message': 'Bot not found'}), 404
        
        monitor = bot_monitors[bot_id]
        stats = monitor.get_statistics()
        positions = monitor.get_positions()
...
if __name__ == '__main__':
    ...
    for i, monitor in enumerate(bot_monitors):
        print(f"   {i+1}. {monitor.name} - {monitor.description}")
        print(f"      Database: {monitor.db_path}")
    print()
    print("🌐 Dashboard available at: http://localhost:8080")
    print("🔄 Auto-refreshes every 30 seconds")
    print("📡 API endpoints:")
    print("   - /api/status - Combined status")
    print("   - /api/bot/0 - Bot 1 details")
    print("   - /api/bot/1 - Bot 2 details")
    print("=" * 60)
